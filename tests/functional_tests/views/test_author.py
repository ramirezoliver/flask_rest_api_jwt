...
@pytest.fixture
def valid_token_header(client, database):
...
def tests_create_author(client, database, valid_token_header):
...
def tests_get_authors(client, database, valid_token_header):
    response = client.get('/authors', headers=valid_token_header)
    assert response.status_code == 200


def tests_delete_existing_author(client, database, valid_token_header):
    data = {"name": "J.K. Rowling", "book": "Harry Potter", "country": "U.K."}
    response = client.post('/author',
        headers=valid_token_header,
        json=data)
...
def tests_delete_failed(client, database, valid_token_header):
    existing_author_id = '99'
    response = client.delete('/author/' + existing_author_id,
        headers=valid_token_header)
